...
if plot_on,
    ...
    % KA: for debugging
    set(ax,'fontsize',15,'Layer','top','XLim',[0 ny*dy/1000],'box','on','nextplot','replacechildren')
...
for i=2:length(t), % years
    ...
    if inlet_model_on
        ...
        for j=1:length(inlet_idx), %inlet morphodynamics per inlet
            ...
            % [KA]: shoreline change from inlet sink now spread out along
            % width of inlet +1 cell in both directions
    ...
    if plot_on && mod(i,100)==1,
        ...
        if make_gif,
            ...
            set(ax,'XLim',get(ax,'XLim'),'YLim',[-2000 6000],'Color',[0 0.5 0]);
            %set(ax,'XLim',get(ax,'XLim'),'YLim',get(ax,'YLim'),'Color',[0 0.5 0]);
            delete(hx);

            % KA: changed these because they were buggy
            hh(1) = patch(ax,'XData',[0 0 1000 1000],'YData',[ymin; z./s_background; z./s_background; ymin]);
            hh(2) = patch(ax,'XData',[0 0 y./1000 1000 1000],'YData',[ymin; x_b(1); x_b; x_b(end); ymin]);
            hh(3) = patch(ax,'XData',[0 0 y./1000 1000 1000],'YData',[ymin; x_s(1); x_s; x_s(end); ymin]);
...
if bseed % [KA]
    ...
    b_out.xs = x_s_save(:,1);  % original shoreline position for seeding
    b_out.wave_angle = wave_ang_save;
